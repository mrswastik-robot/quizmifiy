...
const LoadingQuestions = ({ finished }: Props) => {
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      setProgress((prev) => {
        if (finished) return 100;                      //this 'finished' prop is coming from quizCreation.tsx , if the quiz is finished then the progress bar will be 100% and then we have to wait for 1 sec and then the questions will appear 
        if (prev === 100) {
          return 0;
        }
        if (Math.random() < 0.1) {
